...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "velocessor", "tests")); system.time(test_check("velocessor", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran velocessor_$(VERSION).tar.gz
	rm -rf velocessor.Rcheck/
